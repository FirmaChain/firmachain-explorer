...
const Seo = (props: SeoProps) => {
    const { title, description, titleTemplate, openGraph, additionalMetaTags, additionalLinkTags } = props;
    ...
    return (
        <Helmet>
            ...
            {additionalMetaTags?.map((meta, index) => (
                <meta key={`extra-meta-${index}`} {...meta} />
            ))}
            {additionalLinkTags?.map((link, index) => (
                <link key={`extra-link-${index}`} {...link} />
            ))}
        </Helmet>
    );
};
