...
const Multisend = (props: { message: MsgMultiSend }) => {
  ...
  const senderAmount = sender?.coins
    ...
    .reduce(
      (text, value, i, array) =>
        text + (i < array.length - 1 ? ", " : ` ${t("and")} `) + value,
    );
    ...
    <div>
      <Typography>
        <Trans
          ...
          components={[
            <Name address={sender?.address} name={validatorMoniker} />,
            <b />,
          ]}
          values={{
            amount: senderAmount,
      ...
      <div className={classes.multisend}>
        {receivers?.map((x, i) => {
          ...
          const recieverMoniker = recieverUser
            ? recieverUser?.name
            : x?.address;
          return (
            <Typography key={`${x.address}-${i}`}>
              <Trans
                ...
                components={[
                  <Name address={x?.address} name={recieverMoniker} />,
                  <b />,
                ]}
                values={{
