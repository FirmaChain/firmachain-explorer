...
export const getProposalType = (proposalType: string) => {
    ...
    if (
        proposalType === '/cosmos.gov.v1beta1.TextProposal' ||
    ...
    if (proposalType === '/cosmos.params.v1beta1.ParameterChangeProposal' || proposalType.endsWith('.MsgUpdateParams')) {
        type = 'parameterChangeProposal';
    }

    if (proposalType === '/cosmos.upgrade.v1beta1.SoftwareUpgradeProposal' || proposalType.endsWith('.MsgSoftwareUpgrade')) {
        type = 'softwareUpgradeProposal';
    }

    if (proposalType === '/cosmos.upgrade.v1beta1.CommunityPoolSpendProposal' || proposalType.endsWith('.MsgCommunityPoolSpend')) {
...
export const shouldShowData = (status: string) =>
    ['PROPOSAL_STATUS_VOTING_PERIOD', 'PROPOSAL_STATUS_PASSED', 'PROPOSAL_STATUS_REJECTED', 'PROPOSAL_STATUS_FAILED'].includes(status);
